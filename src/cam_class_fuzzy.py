    ...
    def aruco(self, frame, CoordGet, CoordReset, angles_tof):
        ...
        if np.all(ids != None):
            ...
            for i in range(0, ids.size):
                cv2.drawFrameAxes(frame, self.mtx, self.dist, rvecs[i], tvecs[i], 0.1)  # Draw axis
...
END_event.clear()
cam = Camera(15, dir_queue, 'calibration_files/camcalib_webcam.npz', getCoords_event, navigate_event, END_event)
getCoords = False
resetCoords = False
